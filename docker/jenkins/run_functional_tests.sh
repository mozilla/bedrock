...
docker run -v `pwd`/results:/app/results --link bedrock-selenium-hub-${BUILD_NUMBER}:hub -e SELENIUM_HOST=hub -e BASE_URL=http://bedrock:8000 bedrock_functional_tests:$GIT_COMMIT
