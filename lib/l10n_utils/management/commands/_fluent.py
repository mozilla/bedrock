...
def get_migration_context(recipe_or_template, locale="en"):
    ...
    context = MigrationContext(locale, ref_dir, settings.LOCALES_PATH / locale, enforce_translated=no_reference)
    migration.migrate(context)
    return context


def get_lang_files(template, template_str):
    lang_files = [Path(str(template).split("/templates/")[1]).with_suffix(".lang")]
    for add_lang in ADD_LANG_RE.finditer(template_str):
        ...
        for p in re.findall(r'"(.*?)"', add_lang["files"]):
            ...
            lang_files.append(f)
    return OrderedDict((f, f.stem.replace("-", "_")) for f in lang_files)


def trans_to_lang(string):
    string = strip_whitespace(string)
    return TRANS_PLACEABLE_RE.sub(lambda m: "%({})s".format(m.group("var")), string)
