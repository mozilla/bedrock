...
class TestTemplateTagFuncs(TestCase):
    ...
    def test_get_template_tag_set(self, lang_file_tag_set, parse_template_mock, get_template, get_lang_path):
        """Should return a unique set of tags from all lang files."""
        parse_template_mock.return_value = ["dude", "walter"]
        lang_file_tag_set.side_effect = [set(["dude", "donny"]), set(["dude", "uli", "bunny"]), set(["walter", "brandt"])]
        self.assertSetEqual(_get_template_tag_set("stuff", "es"), set(["dude", "walter", "donny", "uli", "bunny", "brandt"]))
...
class TestPOFiles(TestCase):
    good_messages = [
        ["Said angrily, loudly, and repeatedly.", "Is this your homework Larry?"],
        [None, "The Dude minds!"],
    ]
    ...
    def test_parse_po(self):
        ...
        expected = {
            "templates/some_lang_files.html": self.good_messages,
            "templates/firefox/fx.html": [[None, "Find out if your device is supported &nbsp;»"]],
            "bedrock/firefox/templates/firefox/os/notes-1.3.html": [
                [
                    "For bug 982755",
                    "The WebIccManager API, which allows support for multiple sim cards, "
                    "has had updates: iccChangeEvent has been added using using event "
                    'generator <a href="%(url1)s">bug 814637</a>',
                ]
            ],
        }
    ...
    def test_po_to_langfiles(self, langfiles_mock, append_mock):
        ...
        langfiles_mock.return_value = ["some_lang_files", "firefox/fx"]
    ...
    @override_settings(ROOT=ROOT, LOCALES_PATH=LOCALES_PATH, DOTLANG_FILES=DOTLANG_FILES)
    ...
    def test_uses_default_lang_files(self, pl_mock):
        ...
        pot_to_langfiles()
        calls = [(("{0}/locale/templates/{1}.lang".format(ROOT, lf),), {"skip_untranslated": False}) for lf in DOTLANG_FILES]
