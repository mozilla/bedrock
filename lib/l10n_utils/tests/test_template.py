...
class TestL10nBlocks(TestCase):
    def test_l10n_block_locales(self):
        ...
        on the node.
        """
        tree = jinja_env.parse(
            """{% l10n dude locales=ru,es-ES,fr 20121212 %}
...
@patch.object(jinja_env.loader, "searchpath", TEMPLATE_DIRS)
@override_settings(
    LOCALES_PATH=LOCALES_PATH, ROOT=ROOT, ROOT_URLCONF="lib.l10n_utils.tests.test_files.urls", DOTLANG_FILES=["download_button", "main", "footer"]
)
class TestTemplateLangFiles(TestCase):
    def test_added_lang_files(self):
        ...
        assert request.langfiles == ["dude", "walter", "download_button", "main", "footer"]

    @pytest.mark.skip(reason="does not pick up the files from the parent. captured in bug 797984.")
    def test_added_lang_files_inheritance(self):
        ...
        request = type("request", (), {})()
        template.render(request=request)
        assert request.langfiles == ["donnie", "smokey", "jesus", "dude", "walter", "main", "download_button"]
    ...
    def test_lang_files_order(self, translate):
        ...
        translate.assert_called_with(ANY, ["dude", "walter", "some_lang_files", "download_button", "main", "footer"])
    ...
    def test_lang_files_default_order(self, translate):
        ...
        translate.assert_called_with(ANY, ["inactive_de_lang_file", "active_de_lang_file", "download_button", "main", "footer"])
