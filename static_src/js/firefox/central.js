...
$(document).ready(function() {
    ...
    function addHandlers() {
        $('.tip-container').bind('mouseenter focusin', function (e) {
            ...
            $tip.css({'left': left, 'top': top}).animate({'opacity': 1}, 300);
            $this.children('.arrow').animate({'opacity': 1}, 300);
            $this.children('.callout').animate({'opacity': 0}, 300);
            // track hover interaction in GA
            gaTrack(['_trackEvent','/central Interactions','Tour the User Interface Hovers', this.id]);
            ...
            $this.children('.callout').animate({'opacity': 1}, 300);
        });
    }
    ...
    /*
     * Track click interactions on page support links
     */
    function trackSupportInteraction(e) {
        e.preventDefault();
        ...
        if (newTab) {
            gaTrack(['_trackEvent','/central Interactions','link click', $(this).data('id')]);
        } else {
            e.preventDefault();
            gaTrack(['_trackEvent','/central Interactions','link click', $(this).data('id')], callback);
        }
    }
    $('#support-list').on('click', 'a', trackSupportInteraction);

    /*
     * Track click interactions on 5 easy steps pager links
     */
    function trackPagerInteraction() {
        gaTrack(['_trackEvent','/central Interactions','link click', $(this).data('id')]);
    }
    $('#steps-list').on('click', 'a', trackPagerInteraction);

    /*
     * Track video play interactions for each pager section
     */
    function trackVideoPlayInteraction(e) {
        var name = $(this).closest('figure').data('id');
        gaTrack(['_trackEvent', '/central Interactions', 'play', name]);
    }
    $('figure video').on('play', trackVideoPlayInteraction);

    /*
     * Track video pause and ended interactions for each pager section
     */
    function trackVideoStopInteraction(e) {
        var name = $(this).closest('figure').data('id');
        // is video over?
        // 'pause' event fires just before 'ended', so
        // using 'ended' results in extra pause tracking.
        var action = (this.currentTime === this.duration) ? 'finish' : 'pause';
        gaTrack(['_trackEvent', '/central Interactions', action, name]);
    }
    $('figure video').on('pause', trackVideoStopInteraction);
});
