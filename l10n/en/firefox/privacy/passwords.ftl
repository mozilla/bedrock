...
privacy-passwords-firefox-will = { -brand-name-firefox } will display a warning instead of the website if it is known to be a phishing site.
...
privacy-passwords-in-todays-internet = In today’s internet, where thousands of passwords are stolen every day and accounts are traded on the black market, it’s worth the effort to keep your online life safe. When you use { -brand-name-firefox } products, some of the effort is taken off your plate, because all our products are built to uphold our <a { $url_privacy_products }>privacy promise</a>. And { -brand-name-firefox } is always guided by <a { $url_about_manifesto }>{ -brand-name-mozilla }’s mission</a>, the not-for-profit we are backed by, to build a better internet.
