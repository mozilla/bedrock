...
compare-brave-the-brave-browser-like-so = The { -brand-name-brave } browser, like so many others, is built on { -brand-name-google }’s open-source { -brand-name-chromium } code. Open-source means anyone can use the source code and piggy-back on top of it to build whatever they want — like the <a { $opera }>{ -brand-name-opera }</a> and <a { $edge }>{ -brand-name-edge }</a> browsers. But it doesn’t mean that all { -brand-name-chromium }-based browsers are equal or are themselves open source.
