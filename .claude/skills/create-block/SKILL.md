...
Bedrock has no single centralized block registry. Page-content blocks live in a package split by content domain, e.g. `bedrock/mozorg/blocks/` (`common.py`, `advertising.py`, `leadership.py`, `navigation.py`, ...).
...
StreamField definitions themselves stay on the page model in the app's `models.py` (e.g. `AdvertisingIndexPage.sections`), not in a separate file.
...
If the app composes full pages from block fixtures (`bedrock/<app>/fixtures/page_fixtures.py`), add the new block's variants there too. If the app also has a fixture-loading management command for seeding real pages into the Wagtail admin for manual QA, extend it — but only add one if the app doesn't have one and it's actually wanted; mozorg's fixtures are consumed only from tests and have no such command.

## Tests

Follow whichever convention the app already uses, e.g. a dedicated `test_blocks.py` (mozorg).
...
Keep the heading hierarchy correct. One way is a plain Jinja template variable (not a block field), e.g. `block_level`, set in the parent page template before calling `{% include_block %}`:
...
and consumed by the block's own template: `<h{{ block_level }}>`. Any block that renders a heading and can contain child blocks with their own headings must set/increment `block_level` before including them.
