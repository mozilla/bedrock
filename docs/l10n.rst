...
.. important::
    ...
    When defining `[[paths]]` in Fluent configuration files, the order in which they appear
    matters. The last path definition in a file will overwrite any proceeding paths should
    they match, so make sure that broad definitions (such as paths that enable .ftl files
    for all locales) appear first in the list, with more locale specific paths coming after.
