...
l10n String Extraction docs
---------------------------

`Officiial docs <https://mozilla-l10n.github.io/documentation/products/mozilla_org/working_bedrock.html>`_

`Overview of process
<https://mozilla-l10n.github.io/documentation/products/mozilla_org/extract_strings.html#this-is-a-brief-overview-of-the-process>`_

You will need to fork & clone:

- `Bedrock <https://github.com/mozilla>`_

- `Mozilla-|10n <https://github.com/mozilla-l10n/www.mozilla.org>`_

- `Langchecker <https://github.com/mozilla-l10n/langchecker/>`_

1. Extract strings from `Bedrock <https://github.com/mozilla>`_ template.

Run bin/bootstrap.sh to create the locale folder.

- Assuming the template you want to extract strings from is bedrock/mozorg/templates/mozorg/about/history.html

In your local bedrock directory:

.. code-block:: console

    $ source venv/bin/activate
    (venv) $ ./manage.py l10n_extract bedrock/mozorg/templates/mozorg/about/history.html

Does the above give you errors?

.. code-block:: console

    $ pip install -r requirements/dev.txt

**A .lang file will be created inside /locale/templates in bedrock directory.**

Suggestion: remove the templates folder before doing an extraction to start fresh.

2. Create a pull request in `Mozilla-l10n repo <https://github.com/mozilla-l10n/www.mozilla.org>`_ for l10n string review.

Copy your newly created .lang file to your local mozilla-l10n/www.mozilla.org directory in the en-US directory.

In the case of history.lang the folder would be en-US/mozorg/about/history.lang

Notes to include in .lang files:
...
`Example <https://github.com/mozilla-l10n/www.mozilla.org/pull/296/files>`_

- Entries should be 2 spaces apart.

Open pull request for the en-US file which will be reviewed by a member of the localization team.

❗REMEMBER❗: if there are copy change requests you will need to update not only the .lang file but
the template you extracted from. They must match exactly for translations to be populated.

3. Create pull request in `Langchecker <https://github.com/mozilla-l10n/langchecker/>`_

`Install langchecker tool. <https://github.com/mozilla-l10n/langchecker/#installation>`_

Paths will need to be updated in this file:
...
**$local_storage** should be the path to the parent directory containing l10n project directories.

**$override_local** is a more specific way of routing directly to your local path of
mozillal-10n/www.mozilla.org directory.

For the pull request update this file:
...
- **Flags:** a multidimensional array. For each key (tag) there’s an array of locales for which that key is valid.
  You can use ‘all’ to indicate it applies to all other supported locales.

- **priority:** priorities are numeric values assigned to files. 1 is the highest priority, 5 is the lowest.

- **support_locales:** locales that this template should be translated in. Mozorg has a variable you can use if
  you don’t need to be specific: $mozillaorg.

- **deadline:** if the translations need to happen by a certain date you can pass a deadline value.
  Ex: 'deadline'              => '2019-05-03'

Create langchecker pull request with your changes in app/config/sources.inc.php and link to that pull request in
your previous mozilla-l10n/www.mozilla.org pull request.

`Langchecker PR example <https://github.com/mozilla-l10n/langchecker/pull/894/files>`_

`mozilla-l10n/www.mozilla.org PR example <https://github.com/mozilla-l10n/www.mozilla.org/pull/310>`_

4. Populate all other locales.

Once your en-US .lang pull request from step 2 is approved, it’s time to populate the other locales.
...
.. code-block:: console
    ...
    $ ./app/scripts/lang_update firefox/adblocker.lang 0 all

Move over to your bedrock-l10n/www.mozilla.org directory and you should see a lot of new .lang files
created in all declared locales.

Push this change to your pull request in bedrock-l10n/www.mozilla.org

Once both pull requests have been approved and merged it’s time for the final step!

5. Push to prod!

(`Bedrock-l10n repo <https://github.com/mozilla-l10n/bedrock-l10n>`_: the localization production repository.)

On the branch with your .lang changes

.. code-block:: console

    $ git log --oneline

On the line with your commit (the very top) copy the hash.

Looks something like: 12f44da672

(Make sure all your changes are a single commit, if they are multiple you can do this by squashing.)

It is time to do some git foundation before we can push to the production repo.

In your local bedrock-l10n/www.mozilla.org project folder create a new branch:
...
Add a new remote and set this new branch to track the master branch on the l10n production repo.
`$ git remote -v` to verify new remote.

.. code-block:: console

    $ git remote add production https://github.com/mozilla-l10n/bedrock-l10n.git
    $ git branch -u production/master
    Branch 'upstream' set up to track remote branch 'master' from 'production'.
    $ git pull

Your new branch ‘upstream’ should now be up to date with the remote branch of master on the `l10n production
repo <https://github.com/mozilla-l10n/bedrock-l10n>`_, which we gave the remote name of ‘production’

Let’s say 12f44da672 was your commit hash.

.. code-block:: console

    $ git cherry-pick 12f44da672

The cherry-pick command added that commit to your new ‘upstream’ branch, so you should now be one change ahead of
the production repo.

To verify we can run `git log --oneline` and see that your new commit adding a .lang file is at the very top, the
second commit being the `most recent commit on github <https://github.com/mozilla-l10n/bedrock-l10n/commits/master>`_.

**PUSHING TO PROD**

.. code-block:: console

    $ git push production upstream:master

This command pushes the local branch of `upstream` to the `master` branch of the `production` remote.
