...
[ public ] - l10n String Extraction docs
Extracting strings for l10n quick-start.
Official docs
...
1. Extract strings from Bedrock template.
...
2. Create a pull request in Mozilla-l10n repo for l10n string review.
...
❗REMEMBER❗: if there are copy change requests you will need to update not only the .lang file but the template you extracted from. They must match exactly for translations to be populated.




3. Create pull request in Langchecker


Install langchecker tool.


Paths will need to be updated in this file:

settings.inc.php.ini
...
$override_local is a more specific way of routing directly to your local path of mozillal-10n/www.mozilla.org directory.


For the pull request update this file:

app/config/sources.inc.php
...
Flags: a multidimensional array. For each key (tag) there’s an array of locales for which that key is valid. You can use ‘all’ to indicate it applies to all other supported locales.
priority: priorities are numeric values assigned to files. 1 is the highest priority, 5 is the lowest.
support_locales: locales that this template should be translated in. Mozorg has a variable you can use if you don’t need to be specific: $mozillaorg.
deadline: if the translations need to happen by a certain date you can pass a deadline value. Ex: 'deadline'              => '2019-05-03'

Create langchecker pull request with your changes in app/config/sources.inc.php and link to that pull request in your previous mozilla-l10n/www.mozilla.org pull request.

Langchecker PR example

mozilla-l10n/www.mozilla.org PR example
...
Move over to your bedrock-l10n/www.mozilla.org directory and you should see a lot of new .lang files created in all declared locales.
...
(Bedrock-l10n repo: the localization production repository.)
...
Add a new remote and set this new branch to track the master branch on the l10n production repo. `$ git remote -v` to verify new remote.
...
Your new branch ‘upstream’ should now be up to date with the remote branch of master on the l10n production repo, which we gave the remote name of ‘production’
...
The cherry-pick command added that commit to your new ‘upstream’ branch, so you should now be one change ahead of the production repo.


To verify we can run `git log --oneline` and see that your new commit adding a .lang file is at the very top, the second commit being the most recent commit on github.
