...
you can use the `activation_files` parameter. This should be
...
`mozorg/mission.ftl` file remained inactive for German, even
...
`l10n_css` helper function. The CSS may also be loaded
...
`impressum` page for example is only available in German and
...
`l10n_utils.render` in the context using the name
`active_locales`. This will be the full list of available
translations. Use `add_active_locales` if you want to add
...
`page()` helper function in your `urls.py` file,
...
that inherits from `LangFilesMixin` (which it must if you
...
`active_locales` that will be the full list of localizations
...
`add_active_locales` name in all of the same ways as
`active_locales` above. It's a list of locale codes that
...
For the exact content of each `.toml` or `.json`
