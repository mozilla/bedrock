...
describe('geo.js', function () {
    ...
    describe('hasGeoOverride', function () {
        it('should return a geo value when supplied', function () {
            expect(Mozilla.VPN.hasGeoOverride('/products/vpn/?geo=de')).toEqual(
                'de'
            );
        });
        ...
        it('should return false if used in a production domain', function () {
            expect(
                Mozilla.VPN.hasGeoOverride(
                    'https://www.mozilla.org/en-US/products/vpn/?geo=de'
                )
            ).toBeFalsy();
        });
    });
    ...
    describe('onRequestComplete', function () {
        ...
        beforeEach(function () {
            spyOn(Mozilla.VPN, 'getAvailability').and.returnValue(
                'variable-price'
            );
        ...
        it('should show Join the Waitlist buttons if VPN is not available', function () {
            ...
            expect(Mozilla.VPN.setAvailability).toHaveBeenCalledWith(
                'variable-price'
            );
    ...
    describe('getAvailability', function () {
        const availableCountries =
            '|ca|my|nz|sg|gb|gg|im|io|je|uk|vg|as|mp|pr|um|us|vi|at|be|ch|de|es|fr|it|ie|nl|';

        it('should return `available` if matching country code is found', function () {
            expect(
                Mozilla.VPN.getAvailability('us', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('gb', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('at', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('be', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('ch', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('de', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('es', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('fr', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('it', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('ie', availableCountries)
            ).toEqual('available');
            expect(
                Mozilla.VPN.getAvailability('nl', availableCountries)
            ).toEqual('available');
        });

        it('should return `not-available` if no matching country code is found', function () {
            expect(
                Mozilla.VPN.getAvailability('cn', availableCountries)
            ).toEqual('not-available');
        });

        it('should return `unknown` if no country code is supplied', function () {
            expect(
                Mozilla.VPN.getAvailability(null, availableCountries)
            ).toEqual('unknown');
        });
        ...
        it('should return `not-available` if country list is empty', function () {
            expect(Mozilla.VPN.getAvailability('us', '')).toEqual(
                'not-available'
            );
        });
    });
    ...
    describe('updateSubscriptionURL', function () {
        it('should update the subscription plan ID as expected', function () {
            const result = Mozilla.VPN.updateSubscriptionURL(
                'price_1IgnlcJNcmPzuWtRjrNa39W4',
                'https://accounts.firefox.com/subscriptions/products/prod_FiJ42WCzZNRSbS?plan=price_1IgwblJNcmPzuWtRynC7dqQa&entrypoint=www.mozilla.org-vpn-product-page'
            );
            expect(result).toEqual(
                'https://accounts.firefox.com/subscriptions/products/prod_FiJ42WCzZNRSbS?plan=price_1IgnlcJNcmPzuWtRjrNa39W4&entrypoint=www.mozilla.org-vpn-product-page'
            );
        });
        ...
        it('should not change the URL if there is no plan query parameter', function () {
            const result = Mozilla.VPN.updateSubscriptionURL(
                'price_1IgwblJNcmPzuWtRynC7dqQa',
                'https://accounts.firefox.com/subscriptions/products?entrypoint=www.mozilla.org-vpn-product-page'
            );
            expect(result).toEqual(
                'https://accounts.firefox.com/subscriptions/products?entrypoint=www.mozilla.org-vpn-product-page'
            );
