...
@pytest.mark.skip_if_not_firefox(reason='Whatsnew pages are shown to Firefox only.')
@pytest.mark.nondestructive
def test_facebook_container_picto_block_not_displayed_in_zh_tw(base_url, selenium):
    page = FirefoxWhatsNew76Page(selenium, base_url, params='', locale="zh-TW").open()
    assert not page.is_facebook_container_picto_block_displayed
    assert page.are_two_columns_displayed
