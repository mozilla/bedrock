...
def test_get_mobile_modal_displayed(base_url, selenium):
    page = FirefoxWhatsNew73Page(selenium, base_url, locale='sv-SE').open()
    modal = page.click_get_mobile_button()
    assert modal.is_displayed
    assert not page.send_to_device.is_displayed
    assert page.is_mobile_qr_code_displayed
    modal.close()
...
def test_send_to_device_success(base_url, selenium):
    page = FirefoxWhatsNew73Page(selenium, base_url).open()
    modal = page.click_get_mobile_button()
    assert modal.is_displayed
