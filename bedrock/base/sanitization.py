...
from justhtml import JustHTML, SanitizationPolicy, Unwrap, UrlPolicy, UrlRule
...
def strip_all_tags(html: str) -> str:
    ...
    return JustHTML(
        ...
        policy=policy,
        fragment=True,
        transforms=[Unwrap("script, style")],
...
def sanitize_html(html: str, allowed_tags: set, allowed_attributes: dict) -> str:
    ...
    html = _HTML_COMMENT_RE.sub("", html)
    return JustHTML(html, sanitize=True, policy=policy, fragment=True).to_html(pretty=False)
