...
def test_limited_request(req_mock):
    ...
    req_mock.get.assert_called_once_with("some_url", params={"per_page": 10, "page": 1}, timeout=5)


@responses.activate
def test_unlimited_request():
    api_url = api._api_url(TEST_WP_BLOGS["firefox"]["url"], "tags", None)
    responses.add(responses.GET, api_url + "?per_page=100&page=1", match_querystring=True, json=[1], adding_headers={"X-WP-TotalPages": "3"})
    responses.add(responses.GET, api_url + "?per_page=100&page=2", json=[2, 2], match_querystring=True, adding_headers={"X-WP-TotalPages": "3"})
    responses.add(responses.GET, api_url + "?per_page=100&page=3", json=[3, 3, 3], match_querystring=True, adding_headers={"X-WP-TotalPages": "3"})
...
def test_api_url():
    assert api._api_url("https://moz.blog/", "posts", 4) == "https://moz.blog/wp-json/wp/v2/posts/4"
    assert api._api_url("https://moz.blog/", "tags", None) == "https://moz.blog/wp-json/wp/v2/tags/"
    assert api._api_url("https://moz.blog", "media", 55) == "https://moz.blog/wp-json/wp/v2/media/55"
