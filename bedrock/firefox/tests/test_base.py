...
class TestWhatsNew(TestCase):
    ...
    def test_fx_101_0_0_fr(self, render_mock):
        """Should use whatsnew-fx101-vpn-fr template for 101.0 in French"""
        req = self.rf.get("/firefox/whatsnew/")
        req.locale = "fr"
        self.view(req, version="101.0")
        template = render_mock.call_args[0][1]
    ...
    # begin 102.0 whatsnew tests

    def test_fx_102_0_0_en(self, render_mock):
        """Should use whatsnew-fx102-en template for 102.0 in English"""
        req = self.rf.get("/firefox/whatsnew/")
        req.locale = "en-US"
        self.view(req, version="102.0")
        template = render_mock.call_args[0][1]
        assert template == ["firefox/whatsnew/whatsnew-fx102-en.html"]

    def test_fx_102_0_0_de(self, render_mock):
        """Should use whatsnew-fx102-de template for 102.0 in German"""
        req = self.rf.get("/firefox/whatsnew/")
        req.locale = "de"
        self.view(req, version="102.0")
        template = render_mock.call_args[0][1]
        assert template == ["firefox/whatsnew/whatsnew-fx102-de.html"]

    def test_fx_102_0_0_fr(self, render_mock):
        """Should use whatsnew-fx102-fr template for 102.0 in French"""
        req = self.rf.get("/firefox/whatsnew/")
        req.locale = "fr"
        self.view(req, version="102.0")
        template = render_mock.call_args[0][1]
        assert template == ["firefox/whatsnew/whatsnew-fx102-fr.html"]

    # end 102.0 whatsnew tests

    # begin 103.0 whatsnew tests

    def test_fx_103_0_0_en(self, render_mock):
        """Should use whatsnew-fx103-en template for en-US locale"""
        req = self.rf.get("/firefox/whatsnew/")
        req.locale = "en-US"
        self.view(req, version="103.0")
        template = render_mock.call_args[0][1]
        assert template == ["firefox/whatsnew/whatsnew-fx103-en.html"]

    def test_fx_103_0_0_en_v2(self, render_mock):
        """Should use whatsnew-fx103-en-rally template for en-US locale and v=2 in US country"""
        req = self.rf.get("/firefox/whatsnew/?v=2", HTTP_CF_IPCOUNTRY="US")
        req.locale = "en-US"
        self.view(req, version="103.0")
        template = render_mock.call_args[0][1]
        assert template == ["firefox/whatsnew/whatsnew-fx103-en-rally.html"]

    @override_settings(DEV=False)
    def test_fx_103_0_0_gb_en_us(self, render_mock):
        """Should use whatsnew-fx103-gb template in GB with en-US locale"""
        req = self.rf.get("/firefox/whatsnew/", HTTP_CF_IPCOUNTRY="GB")
        req.locale = "en-US"
        self.view(req, version="103.0")
        template = render_mock.call_args[0][1]
        assert template == ["firefox/whatsnew/whatsnew-fx103-gb.html"]

    @override_settings(DEV=False)
    def test_fx_103_0_0_gb_en_gb(self, render_mock):
        """Should use whatsnew-fx103-gb template in GB with en-GB locale"""
        req = self.rf.get("/firefox/whatsnew/", HTTP_CF_IPCOUNTRY="GB")
        req.locale = "en-GB"
        self.view(req, version="103.0")
        template = render_mock.call_args[0][1]
    ...
    def test_fx_103_0_0_de(self, render_mock):
        """Should use whatsnew-fx103-de template for de locale"""
        req = self.rf.get("/firefox/whatsnew/")
        req.locale = "de"
        self.view(req, version="103.0")
        template = render_mock.call_args[0][1]
        assert template == ["firefox/whatsnew/whatsnew-fx103-de.html"]

    def test_fx_103_0_0_fr(self, render_mock):
        """Should use whatsnew-fx103-fr template for fr locale"""
        req = self.rf.get("/firefox/whatsnew/")
        req.locale = "fr"
        self.view(req, version="103.0")
        template = render_mock.call_args[0][1]
        assert template == ["firefox/whatsnew/whatsnew-fx103-fr.html"]

    def test_fx_103_0_0_other_locales(self, render_mock):
    ...
    def test_fx_105_0_0_fr(self, render_mock):
        """Should use whatsnew-fx105-fr experiment template for fr locale"""
        req = self.rf.get("/firefox/whatsnew/")
        req.locale = "fr"
        self.view(req, version="105.0")
        template = render_mock.call_args[0][1]
