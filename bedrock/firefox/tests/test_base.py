...
class TestWhatsNew(TestCase):
    ...
    # begin 89.0 whatsnew tests

    def test_fx_89_0_0_de(self, render_mock):
        """Should use VPN template for 89.0 in German"""
        req = self.rf.get('/firefox/whatsnew/')
        req.locale = 'de'
        self.view(req, version='89.0')
        template = render_mock.call_args[0][1]
        assert template == ['firefox/whatsnew/whatsnew-fx89-de.html']

    def test_fx_89_0_0_fr(self, render_mock):
        """Should use VPN template for 89.0 in French"""
        req = self.rf.get('/firefox/whatsnew/')
        req.locale = 'fr'
        self.view(req, version='89.0')
        template = render_mock.call_args[0][1]
        assert template == ['firefox/whatsnew/whatsnew-fx89-fr.html']

    def test_fx_89_0_0_locale(self, render_mock):
        """Should use standard whatsnew template for 89.0 in other locales"""
        req = self.rf.get('/firefox/whatsnew/')
        req.locale = 'es-ES'
        self.view(req, version='89.0')
        template = render_mock.call_args[0][1]
